...
namespace OpenForum.UnitTests.Controllers
{
	...
	public class RouteTests
	{
		private static HttpContextBase CreateContext(string url)
		{
			...
			request.SetupGet(x => x.AppRelativeCurrentExecutionFilePath).Returns("//");
			request.SetupGet(x => x.PathInfo).Returns(url);
		...
		public void Get_Reply_Path()
		{
			...
			VirtualPathData path = routes.GetVirtualPath(
				new RequestContext(context, routeData),
				new RouteValueDictionary(new { controller = "Forum", action = "Reply", postId = 7, id = 9, title = "a_title" }));

			Assert.AreEqual("/Forum/Reply/7/9/a_title", path.VirtualPath);
		}
