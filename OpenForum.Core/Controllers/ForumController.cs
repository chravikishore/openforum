...
namespace OpenForum.Core.Controllers
{
    public class ForumController : Controller
    {
        ...
        public ForumController() : this (Configurations.Current.ViewModelFactory, Configurations.Current.PostRepository, Configurations.Current.UserRepository)
        {
        }
        ...
        public ActionResult Index(string searchQuery, int? page)
        {
            ...
            IndexViewModel viewModel = new IndexViewModel()
            {
                ...
                Message = message,
                Posts = Page(posts, page, ItemsPerPage),
        ...
        public ActionResult Post(int? id, string title, string body)
        {
            ...
            if (id != null && post.CreatedBy.Id != _userRepository.FindCurrentUser().Id)
            {
                throw new OpenForumException("This post cannont be editted by the current user. (Post Id = {0}; Author = {1}; Current User = {2})", post.Id, post.CreatedBy.Id, _userRepository.FindCurrentUser().Id);
            }
