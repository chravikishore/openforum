...
namespace OpenForum.Core.DataAccess
{
	public class Searcher : ISearcher
	{
		...
		private Analyzer _analyser = new StandardAnalyzer(VERSION);
		...
		public int[] GetSearchResults(string queryText)
		{
			...
			QueryParser parser = new QueryParser(VERSION, DATA_FIELD, _analyser);
		...
		public void IndexPosts(IEnumerable<Post> posts, bool recreateIndex)
		{
			...
			foreach (var item in posts)
			{
				...
				document.Add(new Field(ID_FIELD, item.Id.ToString(), Field.Store.YES, Field.Index.NOT_ANALYZED));
				document.Add(new Field(DATA_FIELD, item.GetFullText(), Field.Store.NO, Field.Index.NOT_ANALYZED));
